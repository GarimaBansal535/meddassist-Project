...
export default function PatientQuestioner()
{
  var dispatch=useDispatch()
    ...
    const [patientId,setPatientId]=useState(userData.useremail)
  ...
  ////////////////////////////////////////////////////////
    ...
    const handleSubmit=async()=>{
     ...
     const submitRecord =async()=>{
      var body={"doctor":doctorId,'userregistration':patientId,'currentdate':`${new Date().getFullYear()}-${new Date().getMonth()+1}-${new Date().getDate()}`,'currenttime':`${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}`,'ansdata':JSON.stringify(answers)};
    ...
    alert('Press Next')
     ...
     const showQuestion=()=>{
       return(
        ...
        <div style={{width:'1000px',height:'50px',marginTop:'15px',fontSize:25}}>Que.[{index+1}]:{questions[index]?.question?.question}</div>
        <div className={classes.box}>
        <div style={{width:'150px',height:'40px',fontSize:20,fontWeight:'bolder' ,marginTop:'5px',marginLeft:'10px'}}>{index+1}:{questions[index]?.subquestiontext}</div>
